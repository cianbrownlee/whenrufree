...
describe("generateSlots", () => {
  it("generates hourly slots across a single day within the window", () => {
    const day = new Date(Date.UTC(2026, 0, 1));
    const slots = generateSlots(day, day, "09:00", "12:00");
  ...
  it("excludes a slot that would start exactly at dayEndTime", () => {
    const day = new Date(Date.UTC(2026, 0, 1));
    const slots = generateSlots(day, day, "09:00", "10:00");
  ...
  it("repeats the same daily window across a multi-day range", () => {
    ...
    const slots = generateSlots(start, end, "09:00", "11:00");
  ...
  it("returns an empty array when the window is shorter than one slot", () => {
    const day = new Date(Date.UTC(2026, 0, 1));
    expect(generateSlots(day, day, "09:00", "09:30")).toEqual([]);
  });
  ...
  it("covers the full 24 hours for an all-day window (00:00-24:00)", () => {
    ...
    expect(slots[slots.length - 1].toISOString()).toBe("2026-01-01T23:00:00.000Z");
  });
});
...
describe("aggregateAvailability", () => {
  ...
  const [slot0900, slot1000] = allSlots;
  ...
  it("leaves a slot nobody picked at count 0 with no names", () => {
    ...
    expect(result.get(slot1000.toISOString())).toEqual({
      count: 0,
      names: [],
    });
  });
